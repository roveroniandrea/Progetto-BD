...
def showFormStats(form_id):
    ...
    try:
        ...
        for question in form.questionsRel:
            ...
            if question.type == QuestionTypeEnum.text:
                question_entry['openAnswers'] = list(map(lambda q: q.openAnswerRel.one().answer, question.answersRel.all()))
            if question.type == QuestionTypeEnum.date:
                question_entry['openAnswers'] = list(map(lambda q: q.dateAnswerRel.one().answer, question.answersRel.all()))
